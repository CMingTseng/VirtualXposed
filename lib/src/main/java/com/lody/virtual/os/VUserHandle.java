...
public final class VUserHandle implements Parcelable {
     ...
     * calling from a user process then we will send it to the caller's user instead of failing with
     * a security exception
     */
    public static final int USER_CURRENT_OR_SELF = -3;

    /**
     * @hide A user handle to indicate that we would like to send to the current user, but if this
     * is calling from a user process then we will send it to the caller's user instead of failing
    ...
    /**
     * First gid for applications to share resources. Used when forward-locking is enabled but all
     * UserHandles need to be able to read the resources.
     *
     * @hide
     */
    public static final int FIRST_SHARED_APPLICATION_GID = 50000;
    /**
     * Last gid for applications to share resources. Used when forward-locking is enabled but all
     * UserHandles need to be able to read the resources.
     *
     * @hide
     */
    ...
    /**
     * Instantiate a new VUserHandle from the data in a Parcel that was previously written with
     * {@link #writeToParcel(Parcel, int)}.  Note that you must not use this with data written by
     * {@link #writeToParcel(VUserHandle, Parcel)} since it is not possible to handle a null
     * VUserHandle here.
     *
     * @param in The Parcel containing the previously written VUserHandle, positioned at the
    ...
    /**
     * Generate a text representation of the vuid, breaking out its individual components -- user,
     * app, isolated, etc.
     *
     * @hide
     */
    ...
    /**
     * Write a VUserHandle to a Parcel, handling null pointers.  Must be read with {@link
    ...
    /**
     * Read a VUserHandle from a Parcel that was previously written with {@link
     * #writeToParcel(VUserHandle, Parcel)}, returning either a null or new object as appropriate.
     *
     * @param in The Parcel from which to read the VUserHandle
     * @return Returns a new VUserHandle matching the previously written object, or null if a null
     * had been written.
