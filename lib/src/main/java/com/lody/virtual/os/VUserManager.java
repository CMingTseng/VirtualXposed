...
public class VUserManager {
     ...
     * accounts. The default value is <code>false</code>.
     * <p/>
     * Type: Boolean
     */
    ...
    /**
     * Key for user restrictions. Specifies if a user is disallowed from changing Wi-Fi access
     * points. The default value is <code>false</code>.
     * <p/>
     * Type: Boolean
     */
    ...
    /**
     * Key for user restrictions. Specifies if a user is disallowed from enabling the "Unknown
     * Sources" setting, that allows installation of apps from unknown sources. The default value is
     * <code>false</code>.
     * <p/>
     * Type: Boolean
     */
    ...
    /**
     * Key for user restrictions. Specifies if a user is disallowed from configuring bluetooth. The
    ...
    /**
     * Key for user restrictions. Specifies if a user is disallowed from removing users. The default
    ...
    /**
     * Returns the user name of the user making this call.  This call is only available to
     * applications on the system image; it requires the MANAGE_USERS permission.
     *
    ...
    /**
     * Return the serial number for a user.  This is a device-unique number assigned to that user;
     * if the user is deleted and then a new user created, the new users will not be given the same
     * serial number.
     *
     * @param user The user whose serial number is to be retrieved.
     * @return The serial number of the given user; returns -1 if the given VUserHandle does not
     * exist.
    ...
    /**
     * Return the user associated with a serial number previously returned by {@link
     * #getSerialNumberForUser(VUserHandle)}.
     *
     * @param serialNumber The serial number of the user that is being retrieved.
     * @return Return the user associated with the serial number, or null if there is not one.
    ...
    /**
     * Enable or disable the use of a guest account. If disabled, the existing guest account will be
     * wiped.
     *
    ...
    /**
     * Returns the maximum number of users that can be created on this device. A return value of 1
     * means that it is a single user device.
     *
