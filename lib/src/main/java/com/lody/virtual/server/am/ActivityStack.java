...
/* package */ class ActivityStack {
    ...
    /**
     * App started in VA may be removed in OverView screen, then AMS.removeTask will be invoked, all
     * data struct about the task in AMS are released, while the client's process is still alive. So
     * remove related data in VA as well. A new TaskRecord will be recreated in `onActivityCreated`
     */
