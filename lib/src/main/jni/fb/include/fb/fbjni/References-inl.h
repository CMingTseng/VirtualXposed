...
namespace facebook {
    namespace jni {
        ...
        namespace detail {
            ...
            namespace {
                ...
                void StaticAssertValidRepr() noexcept {
                    ...
                    static_assert(IsPlainJniReference<JniType<Repr>>(),
                                  "T must be a JNI reference");
        ...
        template<typename T>
        inline enable_if_t<IsPlainJniReference<T>(), alias_ref < T>>
        wrap_alias(T
        ref) noexcept {
        return
        alias_ref<T>(ref);
...
// more maintainable (DRY), or less (too clever/confusing.).
    ...
    enable_if_t<IsPlainJniReference<T>(), decltype(static_ref_cast < T > (ref))> {
        ...
        static alias_ref <jclass> target_class = findClassStatic(
