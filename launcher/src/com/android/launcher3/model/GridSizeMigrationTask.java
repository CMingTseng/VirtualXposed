...
public class GridSizeMigrationTask {
    ...
    /**
     * To migrate hotseat, we load all the entries in order (LTR or RTL) and arrange them in the
     * order in the new hotseat while keeping an empty space for all-apps. If the number of entries
     * is more than what can fit in the new hotseat, we drop the entries with least weight. For
     * weight calculation {@see #WT_SHORTCUT}, {@see #WT_APPLICATION} & {@see #WT_FOLDER_FACTOR}.
     *
     ...
     * pick the one which causes the least data loss: {@link #tryRemove(int, int, int, ArrayList,
     * float[])} 2) Maintain a list of all lost items before this screen, and add any new item lost
     * from this screen to that list as well. 3) If all those items from the above list can be
     * placed on this screen, place them (otherwise they are placed on a new screen).
     */
