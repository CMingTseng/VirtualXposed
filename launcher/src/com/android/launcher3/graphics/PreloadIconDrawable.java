...
public class PreloadIconDrawable extends FastBitmapDrawable {
    ...
    /**
     * Runs the finish animation if it is has not been run after last call to {@link
     * #onLevelChange}
     */
    ...
    /**
     * Sets the internal progress and updates the UI accordingly for progress <= 0: - icon in the
     * small scale and disabled state - progress track is visible - progress bar is not visible for
     * 0 < progress < 1 - icon in the small scale and disabled state - progress track is visible -
     * progress bar is visible with dominant color. Progress bar is drawn as a fraction of {@link
     * #mScaledTrackPath}.
     *
     * @see PathMeasure#getSegment(float, float, Path, boolean) for 1 <= progress < (1 +
     * COMPLETE_ANIM_FRACTION) - we calculate fraction of progress in the above range - progress
     * track is drawn with alpha based on fraction - progress bar is drawn at 100% with alpha based
     * on fraction - icon is scaled up based on fraction and is drawn in enabled state for progress
     * >= (1 + COMPLETE_ANIM_FRACTION) - only icon is drawn in normal state
     */
